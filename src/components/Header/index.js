...
class Header extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <div className="navbar  header-container navbar-expand-lg">
        <img className="navbar-brand" src={require('../../images/hydro.svg')} alt="hydro" />
        <div className="dropdown  navbar-nav mr-auto">
          <button
            className="btn header-dropdown-btn header-dropdown dropdown-toggle"
            type="button"
  ...
  renderAccount() {
    ...
    if ((isLoggedIn && address) || isLocked) {
      ...
      return (
        <button className="btn header-connect-btn" style={{ marginLeft: 12 }} onClick={() => dispatch(loginRequest())}>
          connect
        </button>
      );
    } else {
