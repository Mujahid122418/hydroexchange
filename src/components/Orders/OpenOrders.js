          ...
          <tbody>
            {orders
              ...
              .map(([id, order]) => {
                ...
                return (
                  <tr key={id}>
                    <td style={{color:'#9b9b9b'}} className="pair-column">{order.marketID}</td>
                    <td style={{color:'#9b9b9b'}} className={order.side === 'sell' ? 'text-danger' : 'text-success'}>{order.side}</td>
                    <td style={{color:'#9b9b9b'}} className="text-right">{order.price.toFixed(currentMarket.priceDecimals)}</td>
