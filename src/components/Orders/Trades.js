          ...
          <tbody>
            {trades
              ...
              .map(([id, trade]) => {
                ...
                return (
                  <tr key={id}>
                    <td  style={{color:'#9b9b9b'}} className="pair-column">{trade.marketID}</td>
                    <td  style={{color:'#9b9b9b'}} className={`${side === 'sell' ? 'text-danger' : 'text-success'}`}>{side}</td>
                    <td  style={{color:'#9b9b9b'}} className={`text-right${side === 'sell' ? ' text-danger' : ' text-success'}`}>
                      {new BigNumber(trade.price).toFixed(currentMarket.priceDecimals)}
                    </td>
                    <td  style={{color:'#9b9b9b'}} className="text-right">
                      {new BigNumber(trade.amount).toFixed(currentMarket.amountDecimals)} {symbol}
                    </td>
                    <td  style={{color:'#9b9b9b'}} className={className}>{status}</td>
                  </tr>
                );
              })}
          </tbody>
