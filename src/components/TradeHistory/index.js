...
class TradeHistory extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <div style={{background:'#2b3247'}} className="trade-history flex-1 position-relative overflow-hidden" ref={ref => this.setRef(ref)}>
              ...
              .map(([id, trade]) => {
                ...
                return (
                  <tr key={trade.id}>
                    ...
                    <td style={{color:'#9b9b9b'}} className="text-right">{new BigNumber(trade.amount).toFixed(currentMarket.amountDecimals)}</td>
                    <td   style={{color:'#9b9b9b'}} className="text-secondary">{moment(trade.executedAt).format('hh:mm:ss')}</td>
                  </tr>
                );
              })}
          </tbody>
